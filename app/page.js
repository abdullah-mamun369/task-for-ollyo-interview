...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const shuffle = () => {
      const newShuffledData = (originalImage.length > 0) && shuffleArray(originalImage);
            ...
            <div className='group'>
              <div className='border rounded-lg relative overflow-hidden'>
                ...
                <div className='absolute h-full w-full bg-black/40 flex justify-end p-5 -bottom-10 group-hover:bottom-0 opacity-0 group-hover:opacity-100 transition-all duration-300'>
                  <input onChange={handleSelected} className='form-checkbox h-6 w-6 rounded-full text-purple-600 bg-purple-600' type="checkbox" />
                </div>
              </div>
            </div>
          </div>
          {shuffledData &&
            shuffledData.map(product => <Product
              key={product.id}
              product={product}
